...
const getGoodsById = async () => {
  ...
  ruleForm.sale=res.sale
  ruleForm.content = res.content;
  ruleForm.status = res.status + "";
  fileList.value[0].name = res.img
  fileList.value[0].url = CONFIG.SERVER_URL + "/upload/img/goods/" + res.img
  //@ts-ignore
  wangEditor.value.setHtml(res.content)
  getSpecifications(res.cid)
...
const submitForm = async () => {
  ...
  //if (ruleForm.img == "") return ElError("请上传商品主图")
