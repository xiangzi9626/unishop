...
const getData=async ()=>{
  ...
  let row:any=await request.post("/admin/goods/category/get?id="+id)
 ruleForm.pid=row.pid+""
  ruleForm.name=row.name
  ruleForm.img=row.img
  fileList.value[0].name = row.img
  fileList.value[0].url = CONFIG.SERVER_URL + "/upload/img/goods/" + row.img
  const res:any=await request.post("/admin/goods/category/list")
  data.selectCategory=buildTree(res)
}
const ruleForm:any=reactive({
  pid:"",
  categoryId:useRoute().query.id,
  name:"",
  img:""
})
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      request.post("/admin/goods/category/edit",ruleForm).then((res: any) => {
        if (res.code == 200) {
           ElSuccess("提交成功")
