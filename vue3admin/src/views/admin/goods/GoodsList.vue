        ...
        <template #default="scope">
          <div class="goods-box">
            <span><img style="max-width:100px;max-height:100px"
                       :src="CONFIG.SERVER_URL+'/upload/img/goods/'+scope.row.img.split(',')[0]"/></span>
            &nbsp;
            <span><p style="width:200px">{{ scope.row.title }}</p></span>
          </div>
        </template>
      </el-table-column>
...
const showDialog = () => {
  ...
  //@ts-ignore
  let checkedArr = data.list.filter((item:any)=> item.checked)
...
const batchDelete = async () => {
  ...
  const res: any = await request.post("/admin/goods/batch_delete", idArr)
  if (res.code == 200) {
    ElSuccess("删除成功")
    data.currentPage = 1
...
const jump = () => {
  router.push("/admin/goods/add")
}
const jumpEdit = (row: any) => {
  router.push("/admin/goods/edit?id=" + row.id)
}
const getData = (currentPage: number, pageSize: number) => {
  request.post("/admin/goods/list", {currentPage: currentPage, pageSize: pageSize}).then((res: any) => {
...
const handleDelete = (row: any) => {
  request.post("/admin/goods/delete", {id: row.id}).then((res: any) => {
    if (res.code == 200) {
      ElSuccess("删除成功")
      getData(data.currentPage, data.pageSize)
    } else {
